...
const ManagerDiaryPage = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      const response = await api.get('/posts', {
        params: { boardType: 'manager-diary', page: currentPage, limit: 15 }
      })
  ...
  return (
    <PageLayout isLoggedIn={isLoggedIn} onLogin={handleLogin}>
      <h1 className="page-title">실장일기</h1>
      ...
      <div className={`board-header ${isLoggedIn && hasShopLevel ? '' : 'no-button'}`}>
        ...
        {isLoggedIn && hasShopLevel && (
          <Link to="/manager-diary/write" className="btn-write">글쓰기</Link>
        )}
      </div>
...
export default ManagerDiaryPage
